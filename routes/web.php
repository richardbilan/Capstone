...
Route::middleware([App\Http\Middleware\Authenticate::class])->group(function () {
    ...
    Route::get('/home', function () {
        ...
        $pwdTotal = (clone $pwdQuery)->count();
        $pwdFemale = (clone $pwdQuery)->where('gender', 'Female')->count();
        $pwdMale = (clone $pwdQuery)->where('gender', 'Male')->count();
        $maternalCount = \App\Models\Resident::whereNotNull('maternal_status')
            ->where('maternal_status','!=','')
            ->where('maternal_status','!=','None')
            ->count();
        $soloParentCount = \App\Models\Resident::whereNotNull('solo_parent')
            ->where('solo_parent','!=','')
            ->count();
    ...
    // Admin Barangay Hazard Map (interactive hazard map)
...
Route::get('/evacuation', function (Request $request) {
    ...
    // Optional coordinates for direct routing
    $mouLat = $request->query('lat');
    $mouLng = $request->query('lng');
    return view('hazard.evacuation', [
        'mouAddress' => $mouAddress,
        'mouLat' => $mouLat,
        'mouLng' => $mouLng,
    ]);
})->name('evacuation');
...
Route::middleware([App\Http\Middleware\Authenticate::class])->group(function () {
    Route::get('/api/hazards', [HazardGeometryController::class, 'index'])->name('hazards.index');
    Route::post('/api/hazards', [HazardGeometryController::class, 'store'])->name('hazards.store');
